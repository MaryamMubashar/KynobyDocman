...
async function uploadDocuments(unattended=false,folderID=false,limit=1){if(unattended){(0,storage.XO)("KYNOBY_UNATTENDED","true");(0,storage.XO)("KYNOBY_UNATTENDED_FOLDER",folderID)}const allLetters=document.querySelectorAll("ul#document_list > li > a");if(!allLetters||allLetters.length===0){alert("No letters found on the page!");return}const lettersToProcess=Array.from(allLetters).slice(0,limit);if(lettersToProcess.length===0){alert("No valid letters selected!");return}for(let idx=0;idx<lettersToProcess.length;idx++){const letter=lettersToProcess[idx];let failedCount=parseInt((0,storage.bQ)("KYNOBY_FAILED_COUNT"))||0;if(!window.location.href.includes("DocumentViewer/Filing")){alert("You're on the wrong page!");return}
// if (!window.location.href.includes("DocumentViewer/Filing")) {
//   alert("You're on the wrong page!");
//   return;
// }
// let failedCount = parseInt(getData("KYNOBY_FAILED_COUNT")) || 0;
// let letter = unattended
//   ? document.querySelectorAll("ul#document_list > li > a")[failedCount]
//   : document.querySelector("ul#document_list > li > a.active") ||
//     document.querySelector("ul#document_list > li > a");
if(!letter){(0,storage.XO)("KYNOBY_UNATTENDED","false");(0,storage.XO)("KYNOBY_UNATTENDED_FOLDER","false");
...
progressPane.updateStep(letter,"Handover to File Document","success");result.addRes("Handover to File Document","success").set("upload documents",true);Constants.ai&&$("#action_file").click();await(0,helpers.cb)(2e3)}alert(`Processed ${lettersToProcess.length} letters successfully.`)}async function fileDocument(){
...
$("#extractXfromSelected").on("click",(()=>(0,_handlers_extractor_js__WEBPACK_IMPORTED_MODULE_1__.W)(true)));$("#uploadSingle").on("click",(()=>{const count=parseInt(prompt("How many letters do you want to upload?","1"),10);if(!isNaN(count)&&count>0){(0,_utils_storage_js__WEBPACK_IMPORTED_MODULE_4__.XO)("KYNOBY_FAILED_COUNT",0);(0,_handlers_uploader_js__WEBPACK_IMPORTED_MODULE_2__.ge)(false,false,count)}}));
// $("#uploadAll").on("click", () => {
//   setData("KYNOBY_FAILED_COUNT", 0);
//   uploadDocuments(
//     true,
//     document.querySelector("#folders_list a.active").getAttribute("data-id")
//   );
// });
$("#addCodes").on("click",(()=>(0,_handlers_uploader_js__WEBPACK_IMPORTED_MODULE_2__.Eo)()));$("#moveLetters").on("click",(()=>(0,_handlers_autoMove_js__WEBPACK_IMPORTED_MODULE_3__.p)()));$("#clearLocalStorage").on("click",(()=>{(0,_utils_storage_js__WEBPACK_IMPORTED_MODULE_4__.N1)()}));$("#changeClient").on("click",(()=>(0,_toolbar_createOnboardingForm_js__WEBPACK_IMPORTED_MODULE_7__.A)()));__webpack_async_result__()}catch(e){__webpack_async_result__(e)}}),1)},529:(__unused_webpack___webpack_module__,__webpack_exports__,__webpack_require__)=>{
/* harmony export */__webpack_require__.d(__webpack_exports__,{
